...
import { useBaixarContaReceber } from '../hooks/useContasReceber'
...
export function BaixaContaReceberModal({ conta, onOpenChange }: BaixaContaReceberModalProps) {
  const { funcionario } = usePermissions()
  const restante = conta ? conta.valor - conta.valorRecebido : 0
          ...
          <div className="space-y-3">
            <div className="rounded-md border p-3 text-sm space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span>{formatCurrency(conta.valorRecebido)}</span>
              </div>
            ...
            >
              {baixar.isPending ? 'Salvando...' : 'Confirmar Recebimento'}
            </button>
          </div>
        )}
      </DialogContent>
